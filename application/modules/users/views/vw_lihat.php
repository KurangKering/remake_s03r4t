...
<script>
    ...
    $(function() {
        ...
        t = $("#tbl_users").dataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: {"url": "<?php echo base_url('users'); ?>/ajax_lihat", "type": "POST"},
            columns: [
            {"data" : "nomor_urut" ,
            "orderable": false},
            {"data": "fullname" },
        ...
        $('<button id="tambah" class="btn btn-default">Tambah</button>').click(function(event) {
            /* Act on the event */
            location.href = '<?php echo base_url('users/tambah'); ?>'
    ...
    {
        $.ajax({
            url: '<?php echo base_url('users'); ?>/ajax_detail',
            type: 'POST',
            data: {id : id},
            cache: false,
            dataType: 'html',
    ...
    {
        $.ajax({
            url: '<?php echo base_url('users'); ?>/ajax_delete_users',
            type: 'POST',
            data: {id : id},
            //processData: false, // Don't process the files
            //contentType: false, // Set content type to false as jQuery will tell the server its a query string request
