...
class Users extends MY_Controller {
	...
	public function tambah()
	{
		...
		if ($this->form_validation->run() == true && $id_user = $this->ion_auth->register($identity, $password, $email, $additional_data))
		{
			...
			$this->session->set_flashdata('message', showNotificationToastr('success', 'Berhasil Menambah User'));
			redirect("users/lihat", 'refresh');
		}
	}
...
//Update the groups user belongs to
			...
			if ($res) {
				$this->session->set_flashdata('message', showNotificationToastr('success', 'Berhasil Merubah Data User'));
				redirect('users/lihat','refresh');
			}
			else if ($this->db->error()['code'] == 1062) {
				$this->session->set_flashdata('message', showNotificationToastr('error', 'Duplikat Terdeteksi'));
...
// update the password if it was posted
			...
			if ($res) {
				$this->session->set_flashdata('message', showNotificationToastr('success', 'Berhasil merubah Data Profile'));
				...
				$this->session->set_flashdata('message', showNotificationToastr('error', 'Gagal Merubah User, Cek email'));
